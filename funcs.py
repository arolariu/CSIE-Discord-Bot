...
async def ruleta_func(ctx, amount):
    emoji_dex = \
        {":one:": 1, ":two:": 2, ":three:": 3,
         ":four:": 4, ":five:": 5, ":six:": 6,
...
async def slots_func(ctx, amount):
    emoji_dex = \
        {
            ":apple:": 1,
            ":tangerine:": 2,
...
async def balance_func(bot, ctx, user):
    old_user = None
    if user is not None:
        try:
            old_user = bot.get_user(int(user[3:-1]))
            if old_user is None:
                old_user = bot.get_user(int(user[2:-1]))
        except ValueError:
...
async def donate_func(bot, ctx, user, amount):
    if user is None:
        return await ctx.channel.send("Sigur ai formulat bine comanda?")
    if int(amount) <= 0:
        return await ctx.channel.send("Nu poti sa donezi o suma atat de mica.")
...
async def deposit_func(ctx, amount):
    try:
        total = int(amount)
    except ValueError:
...
async def loan_func(ctx, amount):
    try:
        total = int(amount)
    except ValueError:
...
async def clasament_func(bot, ctx):
    with open("credits.json", "r") as js:
        data = json.load(js)
    ...
    for key, value in top_12.items():
        if iterator == 13:
            return await ctx.channel.send(embed=embed)
        embed.add_field(name=f"Loc #{iterator}: {str(bot.get_user(int(key)))[:-5]}",
...
async def rob_func(bot, ctx, user):
    if user is None:
        return await ctx.channel.send("Pe cine vrei sa furi, mai exact?")
    try:
        old_user = bot.get_user(int(user[3:-1]))
        if old_user is None:
            old_user = bot.get_user(int(user[2:-1]))
...
async def ping_func(bot, ctx, tests):
    try:
        total = int(tests)
    except ValueError:
        return await ctx.channel.send("Sigur ai formulat corect comanda?")
...
async def avatar_func(bot, ctx, user):
    if user is not None:
        old_user = bot.get_user(int(user[3:-1]))
...
async def wiki_func(bot, ctx, *context):
    total = " ".join(context)
    images = []
...
async def ref_func(bot, ctx, *context):
    total = " ".join(context)
